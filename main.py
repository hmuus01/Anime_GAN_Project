#Code obtained from https://github.com/jayleicn/animeGAN with the changes detailed in the report
...
parser.add_argument('--outDir', default='one', help='folder to output images and model checkpoints')
...
###Changes (As mentioned in the report) ####
arg_list = [
     '--dataRoot', '/home/hamzemuuse1/animeGAN-master/data',
     '--workers', '12',
     '--batchSize', '128',
     '--imageSize', '64',
     '--nz', '100',
     ...
     '--cuda', 
     '--ngpu', '1',
     '--netG', 'results_new_14/netG_epoch_119.pth',
     '--netD', 'results_new_14/netD_epoch_119.pth',
     '--outDir', 'results_new_15',
     '--model', '1',
...
#opt.model = nn.DataParallel(opt.model)
# load models 
if opt.model == 1:
...
#TESTING NOISE
#noise = torch.FloatTensor(opt.batchSize, nz, 1, 1).normal_(0, 1).cuda()
...
#with torch.no_grad():
   #noise.resize_(opt.batchSize, nz, 1, 1)
       ...
       # label.data.resize_(batch_size).fill_(real_label - opt.d_labelSmooth) # use smooth label for discriminator
        ...
        print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f D(x): %.4f D(G(z)): %.4f / %.4f Elapsed %.2f s'
              % (epoch, opt.niter, i, len(dataloader),
                 errD.data.item(), errG.data.item(), D_x, D_G_z1, D_G_z2, end_iter-start_iter))
        #if i % 100 == 0:
            #random.randint(0,
        ###Changes (As mentioned in the report) ####
    for j in range(20):
        ...
        if j == 0:
            vutils.save_image(real_cpu[0:64,:,:,:],
             '%s/real_samples_epoch_.png' % opt.outDir, nrow=8)
        fake,_ = netG(fixed_noise)
        vutils.save_image(fake.data[0:64,:,:,:],
                '%s/fake_samples_iter_%03d.png' % (opt.outDir, j), nrow=8)
    if epoch != 0 and  epoch % (opt.niter-1) == 0:
        # do checkpointing
